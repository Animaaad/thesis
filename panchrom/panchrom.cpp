...
Multipole connect_dangles(Multipole& G, int n1, int n2,
    ...
    G[v1].erase(G[v1].begin() + 2);
    G[v1].push_back(u1);
    
    G[v2].erase(G[v2].begin() + 2);
    G[v2].push_back(u2);
    
    G[u1].erase(G[u1].begin() + 2);

    G[u1].push_back(v1);
    G[u2].erase(G[u2].begin() + 2);
...
Graph replacement(Graph& G, std::vector<Multipole> Replacement_multipoles,
                  int bits, std::vector<std::pair<int, int>> matching) {
    Graph Gr;
    int ns[2];
    ...
    for (int i = 0; i < G.size(); i++) {
        ...
        Gr.insert(Gr.end(), Replacement_multipoles[which].begin(),
    ...
    for (int i = 0; i < G.size(); i++) {
        for (int& j : G[i]) {
            if (i < j) {
                ...
                int n1 = ns[which1];
                int n2 = ns[which2];
                if (contains(matching, {i, j}) || contains(matching, {j, i})) {
                    ...
                    connect_dangles(Gr, n1, n2, connector1, connector2);
...
int main() {
    ...
    try {
        ...
        int graph_counter = 0;
        int line_number;
        std::vector<Graph> input = read_ba_file("4regular.ba");
        for (Graph G : input) {
            ...
            for (int i = 0; i < pow(2, 10); i++) {
                ...
                Graph G2 = replacement(G, Rms, bits, matching);
                graph_counter++;
                int n = G2.size();
                file << graph_counter << "\n" << n << "\n";
                for (int i = 0; i < n; i++) {
