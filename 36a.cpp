        ...
        for (int j = 0; j < m; j++)
            for (int k = 0; k < m; k++) {
                ...
                for (int i = 0; i < 2; i++) {
                    ...
                    int maxN = AB.adj.size() + BB.adj.size() + CBp.adj.size();

                    glue2connectors(AB, BB, CA[i], CB[i], 0, n1);
                    glue2connectors(BB, CBp, CB[(i + 1) % 2], CC[i], n1, n2);
                    glue2connectors(CBp, AB, CA[(i + 1) % 2], CC[(i + 1) % 2],
                                    n2, 0);
...
int main() {
    ...
    J.semiedges = {{4, 0}, {6, 0}, {7, 0}, {8, 1}, {10, 1}, {11, 1}};
